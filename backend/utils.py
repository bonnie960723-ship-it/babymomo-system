...
def format_alert_message(rec) -> str:
    ...
    body_fat = getattr(rec, "body_fat", None)
    ...
    is_male = str(gender).upper() in ("M", "男")
    grip_cut = 28.0 if is_male else 18.0
    smi_cut = 7.0 if is_male else 5.7
    chair_cut = 12.0
    walk_cut = 20.0
    if std:
        if std.get("grip", {}).get("op") == "gte":
    ...
    if body_fat is None:
        ...
        fat_ok = 10 <= body_fat <= 20
    else:
        fat_ok = 23 <= body_fat <= 36.9
    bmi_ok = True if bmi is None else (18.5 <= bmi < 24)
    ...
    smi_txt = f"骨骼肌量 SMI：{smi if smi is not None else '-'} kg/m²（標準 ≥ {smi_cut}）"
    fat_txt = f"體脂率：{body_fat if body_fat is not None else '-'} %（參考 {'10～20' if is_male else '23～36.9'}%）"
    bp_txt = f"血壓：{rec.systolic or '-'}/{rec.diastolic or '-'} mmHg（{bp}）"
    bmi_txt = f"BMI：{bmi if bmi is not None else '-'}（標準 18.5～24）"
    ...
    return (
        ...
        f"{mark(fat_ok, fat_txt)}\n"
        f"{mark(bp_ok, bp_txt)}\n"
        f"脈搏：{rec.pulse or '-'} bpm\n"
        f"{mark(bmi_ok, bmi_txt)}\n"
        "────────────────\n"
        f"肌少症分期：{stage}\n"
        f"異常項目數：{abn}\n"
        f"說明：{rec.status or '-'}\n"
